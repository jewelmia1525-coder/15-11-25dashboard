...
// FIX: Import AdminUser type to use in props
import { Message, Role, AdminUser } from '../types';
...
interface MessageCenterModalProps {
    ...
    // FIX: Add adminUsers to the props interface
...
const MessageCenterModal: React.FC<MessageCenterModalProps> = ({ isOpen, onClose, messages, currentUserIdentifier, onSendMessage, onMarkConversationAsRead, role, showToast, initialSelectedUser, adminUsers }) => {
    ...
    const conversationPartners = useMemo(() => {
        ...
        // For admin, the only conversation partner is super-admin
        if (role === 'admin') {
            users.add('super-admin');
            return Array.from(users);
        }

        // For super-admin, all admin users are potential partners
        if (role === 'super-admin') {
            adminUsers.forEach(user => users.add(user.username));
            // Also include anyone from message history who might no longer be an admin
            messages.forEach(msg => {
                if (msg.sender !== 'super-admin') users.add(msg.sender);
                if (msg.recipient !== 'super-admin') users.add(msg.recipient);
            });
        }
        
        return Array.from(users);
    }, [messages, role, adminUsers]);
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            } else if (role === 'super-admin' && conversationPartners.length > 0 && !selectedUser) {
                const defaultUser = conversationPartners[0];
    ...
    }, [isOpen, initialSelectedUser, role, conversationPartners, onMarkConversationAsRead, selectedUser]);
    ...
    const UserListPanel = (
        <div className="w-full lg:w-1/3 border-r border-primary/20 flex flex-col h-full">
            <h3 className="p-4 font-bold text-lg border-b border-primary/20 flex-shrink-0">Conversations</h3>
            <div className="flex-grow overflow-y-auto">
                {conversationPartners.length === 0 && role === 'super-admin' && <p className="p-4 text-sm text-muted-content">No admin conversations yet.</p>}
                {conversationPartners.map(user => (
                    <button key={user} onClick={() => handleSelectUser(user)} className={`w-full text-left p-4 flex justify-between items-center transition-colors ${selectedUser === user && !isMobileView ? 'bg-primary/20' : 'hover:bg-base-300/50'}`}>
    ...
    const ConversationPanel = ({ onBack }: { onBack?: () => void }) => (
        <div className="w-full lg:w-2/3 flex flex-col h-full">
            {selectedUser ? (
                <>
                    <div className="p-4 border-b border-primary/20 flex-shrink-0 flex items-center gap-2">
                        {isMobileView && onBack && (
                             <button onClick={onBack} className="p-2 -ml-2 rounded-full hover:bg-base-300/50">
                    ...
                    <div className="flex-grow p-4 space-y-4 overflow-y-auto">
                        {conversation.map(msg => (
                            <div key={msg.id} className={`flex flex-col ${msg.sender === currentUserIdentifier ? 'items-end' : 'items-start'}`}>
                                ...
                                <p className="text-xs text-muted-content mt-1">{new Date(msg.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                            </div>
                        ))}
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Message Center">
            <div className="flex flex-col lg:flex-row h-[70vh] -m-6 rounded-2xl overflow-hidden">
                {isMobileView ? (
                    !selectedUser ? UserListPanel : <ConversationPanel onBack={() => setSelectedUser(null)} />
                ) : (role === 'admin' ? (
                        <div className="w-full">{ConversationPanel({})}</div>
                    ) : (
                        <>
                            {UserListPanel}
                            {ConversationPanel({})}
