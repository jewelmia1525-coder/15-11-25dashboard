...
const CustomizeDashboardModal: React.FC<CustomizeDashboardModalProps> = ({ isOpen, onClose, layout, setLayout }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            // Ensure all possible modules are present, even if added later
            const currentIds = new Set(layout.map(m => m.id));
            ...
            const updatedLayout = allModules.map(module => {
                const existing = layout.find(l => l.id === module.id);
                return existing ? { ...module, isVisible: existing.isVisible } : module;
            });
            ...
            allModules.forEach(module => {
                if (!currentIds.has(module.id)) {
                    sortedLayout.push(module);
                }
            });

            setLocalLayout(sortedLayout);
        }
    }, [isOpen, layout]);
